...
async function buildProjectImages() {
  ...
  if (files.length === 0) {
    console.log('❌ No .png files found in', inputPath);
    return;
  }
  ...
  for (const file of files) {
    ...
    const projectName = file.replace('.png', '');
    ...
    try {
      ...
      const compressedOutputPath = path.join(
        outputPath,
        `${projectName}-tiny.${imageFormat}`,
      );
      await converted
        .resize({ height: compressedHeight, method: 'scale' })
        .toFile(compressedOutputPath);
      console.log(
        `✅ Created: ${projectName}.${imageFormat} and ${projectName}-tiny.${imageFormat}`,
      );
